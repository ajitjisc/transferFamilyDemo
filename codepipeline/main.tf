...
# Add permissions for CodePipeline to invoke the webhook for Prod
resource "aws_iam_role_policy" "webhook_invocation_policy_prod" {
  name = "WebhookInvocationPolicyProd"
    ...
    Statement = [
      {
        ...
        Resource = aws_codepipeline.transfer_family_pipeline_prod.arn
      }
    ]
  })
}
